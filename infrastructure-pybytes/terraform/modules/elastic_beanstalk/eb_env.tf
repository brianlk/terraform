resource "aws_elastic_beanstalk_application" "this" {
  ...
  description = "${var.tenant} ${var.environment} application"
...
resource "aws_elastic_beanstalk_environment" "this" {
  ...
  name                = "${var.tenant}-${var.environment}-${each.value["id"]}"
  cname_prefix        = "${var.tenant}-${var.environment}-${each.value["id"]}"
  application         = aws_elastic_beanstalk_application.this.name
  solution_stack_name = data.aws_elastic_beanstalk_solution_stack.node18.name

  setting {
    namespace = "aws:ec2:vpc"
    name      = "VPCId"
    value     = var.aws_aws_vpc_id
    resource  = ""
  }
  ...
  setting {
    namespace = "aws:autoscaling:launchconfiguration"
    name      = "EC2KeyName"
    value     = var.aws_key_pair_deployer
    resource  = ""
  }
  ...
  setting {
    namespace = "aws:autoscaling:launchconfiguration"
    name      = "SecurityGroups"
    value     = each.value["mqtt-group"] ? "${var.aws_eb_security_groups_id},${var.aws_mqtt_security_group_id}" : var.aws_eb_security_groups_id
    resource  = ""
  }
...
data "aws_instance" "eb_environment_ec2_instance" {
  ...
  for_each    = { for x in var.aws_eb_environments : x.id => x }
  instance_id = aws_elastic_beanstalk_environment.this[each.value["id"]].instances[0]
}
